...
<div class="popover mds-bootstrap-persian-datetime-picker-popover popover-boomgard" role="tooltip" ${mdDatePickerPopoverFlag} data-placement="bottom">
    <span class="material-icons">arrow_drop_down</span>
    <div class="arrow"></div>       
    <div class="popover-body p-0" data-name="mds-datetimepicker-popoverbody"></div>
    <div class="c-tools">
        <span id="selected_days">{{selectedDaysFormat}}</span>
        <div class="seprate">
            <div class="btn">
                <button id="clear_date" class="mdc-button mdc-ripple-no-line gray-bnt"><span class="mdc-button__ripple"></span>{{clearDateText}}</button>
                <button id="select_date" class="mdc-button mdc-ripple-no-line"><span class="mdc-button__ripple"></span>{{selectDateText}}</button>
            </div>
        </div>
    </div>
...
<td class="border-0" style="{{monthTdStyle}}" {{monthTdAttribute}} data-td-month>
    ...
    function getSelectedDateTimeTextFormatted(setting) {
        ...
        if (setting.rangeSelector && setting.rangeSelectorStartDate != undefined && setting.rangeSelectorEndDate != undefined) {
            if (setting.rangeFromInput == undefined && setting.rangeToInput == undefined)
    ...
    function setSelectedData(setting) {
        if (setting.rangeSelector == true && setting.rangeFromInput != undefined && setting.rangeToInput != undefined) {
            var date = getSelectedDateTimeTextFormatted(setting);
            var splited = date.split('-');
            var $rangeFrom = $(setting.rangeFromInput);
            if ($rangeFrom.length > 0) {
                if ($rangeFrom[0].tagName.toLowerCase() == 'input') {
                    $rangeFrom.val(splited[0]);
                } else {
                    $rangeFrom.text(splited[0]);
                }
            }

            var $rangeTo = $(setting.rangeToInput);
            if ($rangeTo.length > 0) {
                if ($rangeTo[0].tagName.toLowerCase() == 'input') {
                    $rangeTo.val(splited[1]);
                } else {
                    $rangeTo.text(splited[1]);
                }
            }

            if (setting.rangeFromHidden != undefined && setting.rangeToHidden != undefined) {
                var $rangeFromHidden = $(setting.rangeFromHidden);
                var $rangeToHidden = $(setting.rangeToHidden);

                var hDate = getSelectedDateTimeFormatted(setting);
                var splitedHDate = hDate.split('-');
                $rangeFromHidden.val(splitedHDate[0]);
                $rangeToHidden.val(splitedHDate[1]);
            }
        } else {
    ...
    function getWeekDayShortName(englishWeekDayIndex, isGregorian) {
        if (!isGregorian) {
            return shortDayNamesPersianEdited[englishWeekDayIndex];
        }
    ...
    function getYearsToSelectHtml(setting, yearToStart) {
        ...
        for (var i = yearStart; i < yearEnd; i++) {
            ...
            yearsToSelectHtml += `<td class="text-center" ${selectedDateTimeToShowJson.year == i ? 'selected-year' : ''}><button class="btn btn-sm btn-light" type="button" data-changedatebutton data-number="${yearDateNumber}" ${currentYearDisabledAttr}>${yearText}</button></td>`;
    ...
    function getDateTimePickerMonthHtml1(setting, isNextMonth, isPrevMonth) {
        ...
        html = html.replace(/{{selectDateText}}/img, setting.selectDateText);
        html = html.replace(/{{clearDateText}}/img, setting.clearDateText);
        ...
        if (setting.isGregorian) {
            ...
            for (i = 0; i < setting.disabledDates.length; i++) {
                var t = setting.disabledDates[i];
                var date = new Date();
                if (!is_date(t)) {
                    date = new Date(t * 1000);
                } else {
                    date = setting.disabledDates[i];
                }
                disabledDatesNumber.push(convertToNumber1(getDateTimeJson1(date)));
            }
            ...
            for (i = 0; i < setting.disabledDates.length; i++) {
                ...
                disabledDatesNumber.push(convertToNumber1(getDateTimeJsonPersian1(date)));
            }
            for (i = 0; i < setting.specialDates.length; i++) {
        ...
        if (firstWeekDayNumber != 6) {
            if (setting.isGregorian) {
                console.log(firstWeekDayNumber);
                firstWeekDayNumber--;
        ...
        for (i = 1; i <= numberOfDaysInCurrentMonth; i++) {
            ...
            $td = $('<td data-day />')
                .attr('data-number', currentDateNumber)
                .html('<span><i>' + dayNumberInString + '</i></span>');
            ...
            if (setting.rangeSelector) {
                ...
                if (currentDateNumber == rangeSelectorStartDateNumber || currentDateNumber == rangeSelectorEndDateNumber) {
                    ...
                    var RangeStartDate = rangeSelectorStartDateNumber;
                    var RangeEndDate = rangeSelectorEndDateNumber;
                    $td.addClass('range-start');
                    if (RangeEndDate > RangeStartDate) {
                        var x = "";
                        x = $td.attr('data-number');
    ...
    function setSelectedDays(setting, days) {
        var selectedDaysFormat = setting.selectedDaysFormat.replace('{0}', days);
        $('#selected_days').text(selectedDaysFormat);
    }
    //#endregion

    //#region Events

    $(document).on('click', mdDatePickerContainerSelector, function () {

        $('td').removeClass('range-empty');
        $('td.selected-range-days').siblings("td[data-nm='']").addClass('range-empty');
    ...
    $(document).on('click', mdDatePickerContainerSelector + ' [data-day]', function () {
        ...
        if (setting.rangeSelector) { // اگر رنج سلکتور فعال بود
            ...
            if (setting.rangeSelectorStartDate != undefined && setting.rangeSelectorEndDate != undefined) {
                ...
                $this.parents('table:last').find('td.range-start,td.range-end,td.selected-range-days')
            ...
            if (setting.rangeSelectorStartDate == undefined) {
                ...
                setSelectedDays(setting, 1);
                //$this.addClass('range-start');
                start_day = Number($this.attr('data-number'));
                $this.addClass('circle');

                click++;
                if (click >= 2) {
                    $this.parents('table').find('td.range-start,td.range-end,td.selected-range-days,.circle').removeClass('circle');
                    click = 0;
                    $this.addClass('circle');
                }
                var $this = $(this),
                ...
                end_day = Number($this.attr('data-number'));
                // console.log('days: ' , (Number(end_day + 1) - Number(start_day)));
                ...
                var days = calculateTimeDiff(Number(end_day), Number(start_day));
                setSelectedDays(setting, days);
    ...
    $(document).on('click', '#select_date', function () {
        $('.popover-boomgard').removeClass('active');
        if ($('#inputDate1').val() == '' && $('#inputDate1-1').val() == '') {
            $('.date-in-out .mdc-floating-label').removeClass('up-lbl');
            $('.date-in-out .mdc-floating-label').removeClass('mdc-floating-label--float-above');
            $('.date-in-out .mdc-notched-outline').removeClass('mdc-notched-outline--notched')
        }
    });
    ...
    // $(document).on('click', '#clear_date', function() {
    //     var $this = $(this);
    //     var setting = getSetting1($this);
    //     console.log(setting);
    //     if(setting.rangeSelector == true && setting.rangeFromInput != undefined && setting.rangeToInput != undefined) {
    //         //$(setting.rangeFromInput)
    //     }
    //     else {
    //         $(setting.targetTextSelector).MdPersianDateTimePicker('clearDate');
    //     }
    // });
    ...
    $(document).on('mouseenter', mdDatePickerContainerSelector + ' [data-day],' + mdDatePickerContainerSelector + ' [data-nm],' + mdDatePickerContainerSelector + ' [data-pm]', function () {
        var $this = $(this),
            ...
            dateNumber = Number($this.attr('data-number')),
            setting = getSetting1($this);
        //console.log(setting.rangeSelectorStartDate);
        ...
        $allTdDays.removeClass('range-start');
        $allTdDays.removeClass('range-end');
        ...
        if (rangeSelectorEndDateNumber == 0 && rangeSelectorStartDateNumber > 0 && rangeSelectorStartDateNumber > dateNumber) {
            setSelectedDays(setting, 0);
        }
        if (rangeSelectorStartDateNumber == dateNumber) {
            setSelectedDays(setting, 1);
        }
        if (rangeSelectorStartDateNumber > 0 && dateNumber > rangeSelectorStartDateNumber) {
            // console.log('date start =' + rangeSelectorStartDateNumber+'    dateNumber  ='+dateNumber);
            var days = calculateTimeDiff(Number(dateNumber), Number(rangeSelectorStartDateNumber));
            setSelectedDays(setting, days);

            var temp = rangeSelectorStartDateNumber;
            temp = temp + 1;
            $allTdDays.filter('[data-number="' + rangeSelectorStartDateNumber.toString() + '"]:not(.selected-range-days-start-end)').addClass('range-start');
            var i1 = temp;
            for (i1; i1 <= dateNumber; i1++) {
                $allTdDays.filter('[data-number="' + i1.toString() + '"]:not(.selected-range-days-start-end)').addClass('selected-range-days');
            }
            i1 = i1 - 1;
            $allTdDays.filter('[data-number="' + i1.toString() + '"]:not(.selected-range-days-start-end)').addClass('range-end');
    ...
    var methods = {
            ...
            return this.each(function () {
                var $this = $(this),
                    setting = $.extend({
                        ...
                        previousDaysShow: true,
                        nextDaysShow: true,
                        selectDateText: 'select',
                        clearDateText: '',
                        selectedDaysFormat: '{0} شب انتخاب شده',
                        rangeFromInput: undefined,
                        rangeToInput: undefined,
                    }, options);
                ...
                if (setting.inLine) {
                    ...
                    var DonotShowsetPopoverHeaderHtml;
                    if (!setting.setPopoverHeaderHtml) {
                        DonotShowsetPopoverHeaderHtml = true;
                    }
                    popverHtmlTemplate = popverHtmlTemplate.replace(/{{selectDateText}}/img, setting.selectDateText);
                    popverHtmlTemplate = popverHtmlTemplate.replace(/{{clearDateText}}/img, setting.clearDateText);
                    popverHtmlTemplate = popverHtmlTemplate.replace(/{{selectedDaysFormat}}/img, setting.selectedDaysFormat.replace('{0}', '0'));
                    $this.popover({
                        container: '.main-header .box-search .search-info .res-search',
                        content: '',
                        html: true,
                        placement: 'bottom',
                        title: ' ',
                        ...
                        setTimeout(function () {
                            ...
                            if (DonotShowsetPopoverHeaderHtml) {

                            } else {
                                setPopoverHeaderHtml($this, setting.inLine, $(calendarHtml).find('[data-selecteddatestring]').text().trim());
                            }
                            if (setting.editeHtmlTemplate) {
                                dateTimePickerMonthTableHtmlTemplate = setting.htmlTemplate;
                            }
        ...
        clearDate: function () {
            return this.each(function () {
                ...
                if (setting.rangeFromHidden != undefined || setting.rangeToHidden != undefined) {
                    $(setting.rangeFromHidden).val('');
                    $(setting.rangeToHidden).val('');
                }
            });
        },
        setDays: function (date, d) {
            return this.each(function () {
                var $this = $(this),
                    setting = getSetting2($this);
                setSetting1($this, setting);
                // setting.selectedDate = getClonedDate(date);
                // setSelectedData(setting);
                // setSelectedDays(setting, d);

                var currentDate = date;
                setting.rangeSelectorStartDate = currentDate;
                var endDate = date.addDays(d);
                console.log(endDate);
                setting.rangeSelectorEndDate = endDate; //date.addDays(d);

                var $allTdDays = $this.parents('table:last').find('td[data-day]'),
                    disabled = $this.attr('disabled'),
                    dateNumber = Number($this.attr('data-number'));
 ...
 * @returns {Number}
 */
function calculateTimeDiff(d1, d2) {
    let g = d1 > d2 ? d1 : d2;
    let l = d2 < d1 ? d2 : d1;
    let yg = 0,
        mg = 0,
    ...
    let yl = 0,
        ml = 0,
        dl = 0;
    //calculate greate date
    {
        let s = String(g);
        yg = parseInt(s.substr(0, 4));
        mg = parseInt(s.substr(4, 2));
        dg = parseInt(s.substr(6, 2));
    }
    //calculate greate date
    {
        let s = String(l);
        yl = parseInt(s.substr(0, 4));
        ml = parseInt(s.substr(4, 2));
        dl = parseInt(s.substr(6, 2));
    }
    let days = 1;
    while (`${yg}-${mg}-${dg}` != `${yl}-${ml}-${dl}`) {
        days++;
        dl++;
        if (ml < 12) {
            if (dl == 31 && ml >= 7) {
